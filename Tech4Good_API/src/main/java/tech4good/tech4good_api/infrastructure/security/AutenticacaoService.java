...
public class AutenticacaoService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        System.out.println("[DEBUG AUTH] Tentando buscar usuário por ID: " + username);
        try {
            Integer userId = Integer.parseInt(username);
            Voluntario voluntario = voluntarioGateway.buscarPorId(userId);
            ...
            System.out.println("[DEBUG AUTH] ID inválido: " + username);
            throw new UsernameNotFoundException(String.format("Usuario com ID: %s não encontrado", username));
        } catch (Exception e) {
